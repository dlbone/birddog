...
# Output bitrate and channel count for the live stream can be tuned for Pi load.
LIVESTREAM_BITRATE="${LIVESTREAM_BITRATE:-128k}"
LIVESTREAM_CHANNELS="${LIVESTREAM_CHANNELS:-1}"
...
if [ -z "${REC_CARD}" ];then
  ...
  ffmpeg -nostdin -loglevel "$LOGGING_LEVEL" -ac "${CHANNELS}" -i "${SELECTED_RSTP_STREAM}" -acodec libmp3lame \
    -b:a "$LIVESTREAM_BITRATE" -ac "$LIVESTREAM_CHANNELS" -content_type 'audio/mpeg' \
    ${FREQSHIFT_OPT} \
