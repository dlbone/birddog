...
def prompt_for(com_name, sci_name, variant="collage"):
    ...
    if variant == "detail":
        return (
            ...
            "Create an alternate field-guide portrait showing the bird flying in midair, wings spread, not perched or standing. "
            "Show a complete flight pose with the head, wings, tail, legs, and feet visible when anatomically visible in flight; "
            "do not crop the wings, tail, head, or feet. "
            "No branch, no perch, no twig, no leaves, and no ground. "
    ...
    return (
        f"{style}. Subject: {com_name} ({sci_name}). "
        "Depict the bird accurately for species identification. "
        "Create the main collage portrait with the bird perched naturally on a simple narrow branch. "
        "Show the complete bird from beak to tail to feet, including both legs, feet, and toes gripping the branch; "
        "do not crop, cover, or hide the feet. "
        "Include only a short visible branch as the perch under the feet; no leaves, no ground, no nest, and no extra scenery. "
        "Single bird only. The bird must not have a sticker-like outline, die-cut edge, "
        "stroke, halo, border, or artificial contour around the body. "
        "Place it on a flat pure white background (#ffffff) only; do not draw a checkerboard, "
        "Photoshop transparency grid, black background, or colored background."
